...
public record BoundingBox(Vec3 min, Vec3 max) {
    ...
    public BoundingBox(double size) {
        this(new Vec3(-size, -size, -size), new Vec3(size, size, size));
    }
    ...
    public Pair split(double t, Axis axis) {
        return switch (axis) {
            case X -> new Pair(
                new BoundingBox(min, new Vec3(t, max.y(), max.z())),
                new BoundingBox(new Vec3(t, min.y(), min.z()), max)
            );
            case Y -> new Pair(
                new BoundingBox(min, new Vec3(max.x(), t, max.z())),
                new BoundingBox(new Vec3(min.x(), t, min.z()), max)
            );
            case Z -> new Pair(
                new BoundingBox(min, new Vec3(max.x(), max.y(), t)),
                new BoundingBox(new Vec3(min.x(), min.y(), t), max)
            );
        };
    }

    public Axis longest() {
        Vec3 size = Vec3.subtract(max, min);
    ...
    public BoundingBox splitLeft() {
        Vec3 size2 = Vec3.subtract(
            Vec3.divide(Vec3.subtract(max, Vec3.zero), 2),
            Vec3.divide(Vec3.subtract(min, Vec3.zero), 2)
        );
        if (size2.x() >= size2.y() && size2.x() >= size2.z()) {
            return new BoundingBox(
                min,
                new Vec3(min.x() + size2.x(), max.y(), max.z())
            );
        } else if (size2.y() >= size2.x() && size2.y() >= size2.z()) {
            return new BoundingBox(
                min,
                new Vec3(max.x(), min.y() + size2.y(), max.z())
            );
        } else {
            return new BoundingBox(
                min,
                new Vec3(max.x(), max.y(), min.z() + size2.z())
            );
        }
    }
    ...
    public BoundingBox splitRight() {
        ...
        if (size2.x() >= size2.y() && size2.x() >= size2.z()) {
            return new BoundingBox(
                new Vec3(min.x() + size2.x(), min.y(), min.z()),
                max
            );
        } else if (size2.y() >= size2.x() && size2.y() >= size2.z()) {
            return new BoundingBox(
                new Vec3(min.x(), min.y() + size2.y(), min.z()),
                max
            );
        } else {
            return new BoundingBox(
                new Vec3(min.x(), min.y(), min.z() + size2.z()),
                max
            );
        }
    }
     ...
     */
    public BoundingBox transform(Mat4x4 xform) {
        BoundingBox result = BoundingBox.empty;

        result = result.extend(Mat4x4.multiplyPoint(xform, min));
        result = result.extend(
            Mat4x4.multiplyPoint(xform, new Vec3(min.x(), min.y(), max.z()))
        );
        result = result.extend(
            Mat4x4.multiplyPoint(xform, new Vec3(min.x(), max.y(), min.z()))
        );
        result = result.extend(
            Mat4x4.multiplyPoint(xform, new Vec3(min.x(), max.y(), max.z()))
        );
        result = result.extend(
            Mat4x4.multiplyPoint(xform, new Vec3(max.x(), min.y(), min.z()))
        );
        result = result.extend(
            Mat4x4.multiplyPoint(xform, new Vec3(max.x(), min.y(), max.z()))
        );
        result = result.extend(
            Mat4x4.multiplyPoint(xform, new Vec3(max.x(), max.y(), min.z()))
        );
        result = result.extend(Mat4x4.multiplyPoint(xform, max));
    ...
    public boolean intersect(
        ...
        if (this.contains(Vec3.add(origin, Vec3.multiply(direction, tMin)))) return true;
        if (this.contains(Vec3.add(origin, Vec3.multiply(direction, tMax)))) return true;
     ...
     */
    public Vec3 center() {
        return Vec3.interpolate(max, min, 0.5);
    }
    ...
    public BoundingBox scale(double factor) {
        var c = center();
        return new BoundingBox(
            Vec3.add(Vec3.multiply(factor, Vec3.subtract(min, c)), c),
            Vec3.add(Vec3.multiply(factor, Vec3.subtract(max, c)), c)
        );
    }
