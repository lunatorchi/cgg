...
public class Wavefront {
	...
	public static Map<String, ObjectData> loadObjectData(String filename) {
		...
		try (var input = new FileInputStream(filename)) {
			...
			for (var v : model.getVertices()) vertices.add(new Vec3(v.x, v.y, v.z));

			final List<Vec3> normals = new ArrayList<>();
			for (var n : model.getNormals()) normals.add(new Vec3(n.x, n.y, n.z));

			final List<Vec2> uvs = new ArrayList<>();
			for (var t : model.getTexCoords()) uvs.add(new Vec2(t.u, t.v));
	...
	private static Color toColor(MTLColor mtlc) {
		return new Color(mtlc.r, mtlc.g, mtlc.b);
	}
	...
	private static MaterialData defaultMaterialData = new MaterialData(
		...
		new ConstantColorSampler(new Color(0.1, 0.1, 0.1)),
		Color.gray,
		new ConstantColorSampler(Color.gray),
		Color.gray,
		new ConstantColorSampler(Color.gray),
		0
	);
